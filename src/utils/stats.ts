...
import { isInRange, epoch2datetime, datetime2epoch, startOfDay, endOfDay } from './time'
 ...
 */
export function dailyFocusTimeBounds(entries: FocusSession[], dates: CalendarDateTime[]) {
  let workTimePerDay = dates.map(date => {
    return getKPIs(entries, [startOfDay(date), endOfDay(date)], { minutes: true, precision: 3 }).total;
  })
 ...
 * @returns 
 */
export function fakeSessionData(start: CalendarDateTime, end: CalendarDateTime, maxSessions: number): FocusSession[] {
  ...
  for (let currentDate = datetime2epoch(start); currentDate <= datetime2epoch(end); currentDate += oneDay) {
